...
import static com.example.qurrah.LocationTrackingServices.LocationTracking.JOB_STATE_CHANGED;
import static com.example.qurrah.LocationTrackingServices.LocationTracking.LOCATION_ACQUIRED;
...
public class LocationJobService extends JobService implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    public static final String CHANNEL_app_ID = "track_marty";
...
//                            startService(i1); //i have disabled the call as the server URL in intent service is dummy URL. Change the URL to your server URL and call this intent service
    ...
    private void createNotification() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            mChannel.setDescription("This is channel app ");
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            notification = mBuilder.setSmallIcon(R.drawable.qurrah).setTicker("Tracking").setWhen(0)
                    .setAutoCancel(false)
                    .setCategory(Notification.EXTRA_BIG_TEXT)
                    .setContentTitle(" ")
                    .setContentText("جاري تتبع موقعك الجغرافي")
                    .setSound(RingtoneManager.getDefaultUri(RingtoneManager.TYPE_NOTIFICATION))
                    .setColor(ContextCompat.getColor(getBaseContext(),R.color.colorPrimaryDark))
                    .setStyle(new Notification.BigTextStyle()
                            .bigText("جاري تتبع موقعك الجغرافي"))
                    .setChannelId(CHANNEL_app_ID)
                    .setShowWhen(true)
                    .setOngoing(true)
                    .build();
        }else{
            notification = mBuilder.setSmallIcon((Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP)? R.drawable.qurrah:R.mipmap.ic_launcher).setTicker("Tracking").setWhen(0)
                    .setAutoCancel(false)
                    .setCategory(Notification.EXTRA_BIG_TEXT)
                    .setContentTitle(" ")
                    .setContentText("جاري تتبع موقعك الجغرافي")
...
//            NotificationChannel mChannel = new NotificationChannel("track_marty", "Track", NotificationManager.IMPORTANCE_MIN);
//            mChannel.setShowBadge(false);
...
//        }
        ...
        startForeground(100,notification); //for foreground service, don't use 0 as id. it will not work.
    }
